 ...
 *
 * Ported from consult/acp/run.js::sanitize(), plus ghu_/ghs_/ghr_ tokens and
 * complete JWTs, which transcripts must not keep.
 *
 * Usage:
...
const REDACTION_PATTERNS = [
  ...
  [/gh[usr]_[a-zA-Z0-9]{36,}/g, '[REDACTED_TOKEN]'],
  ...
  // A complete JWT (header.payload.signature) is a bearer credential. A lone
  // base64 header without the other two parts is left alone below.
  [/eyJ[A-Za-z0-9_-]{8,}\.eyJ[A-Za-z0-9_-]{8,}\.[A-Za-z0-9_-]{6,}/g, '[REDACTED_JWT]'],
];
...
function redact(text) {
  ...
  result = result.replace(HIGH_ENTROPY_PATTERN, (match) => {
    ...
    if (match.startsWith('eyJ')) return match; // lone JWT header segment; full JWTs are caught above
